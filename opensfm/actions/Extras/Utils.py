...
def findHomography_v2(matches, kp1, kp2, R, t, K, flag):
    ...
    for match in matches:
        ...
        if flag == 'no_order':
            baseImage_pts.append(kp1[match[0]])
            secImage_pts.append(kp2[match[1]])
    
        if flag == 'order1':
            baseImage_pts.append(kp1[match[0].queryIdx])
            secImage_pts.append(kp2[match[0].trainIdx])
    ...
    print('--- HOMOGRAPHY - KP - SHAPES ---')
    print(baseImage_pts.shape, secImage_pts.shape)

    print("******************************")
    print(" ------- FIND-Homography ------- ")
    print(baseImage_pts, secImage_pts, baseImage_pts.shape, secImage_pts.shape)
    print("******************************")

    # H = K @ (R + t @ np.array([[0, 0, 1]])) @ np.linalg.inv(K)

    # H = H / H[2, 2]

    # return H, np.ones(len(matches), dtype=np.uint8)

    (refinedH, status) = cv2.findHomography(secImage_pts, baseImage_pts, cv2.RANSAC, 4.0, mask=None, maxIters=2000, confidence=0.995)
    return refinedH, status

def findHomography_v3(prev_R, prev_t, curr_R, curr_t, k):
    
    print(' ---- Using findHomography_v3  ---- ')

    H = manual_compute_homography_v2(prev_R, prev_t, curr_R, curr_t, k)
...
def getNewFrameSizeAndMatrix(homographymatrix, shape_img2, shape_img1, is_direct = False):
...
def manual_compute_homography_v2(prev_R, prev_t, curr_R, curr_t, camera_matrix):

    rel_R = curr_R @ prev_R.T
    rel_t = curr_t - rel_R @ prev_t

    print()
    print(rel_R.shape, rel_t.shape, camera_matrix.shape)
    print()

    homography_matrix = camera_matrix @ (rel_R - rel_t @ np.array([[0, 0, 1]]) / camera_matrix[2, 2]) @ np.linalg.inv(camera_matrix)
