...
### Understanding --> pan=yaw, pitch=pitch, roll=roll
def rotation_from_ptr(yaw: float, pitch: float, roll: float) -> np.ndarray:
    """World-to-camera rotation matrix from yaw, pitch and roll. (rotation-order: ZYX)"""
    R1 = rotation_from_angle_axis(np.array([0.0, 0.0, roll]))
    R2 = rotation_from_angle_axis(np.array([pitch + np.pi / 2, 0.0, 0.0]))
    R3 = rotation_from_angle_axis(np.array([0.0, 0.0, yaw]))
...
def ptr_from_rotation(
    ...
    """yaw pitch and roll from camera rotation matrix"""
    yaw = yaw_from_rotation(rotation_matrix)
    pitch = pitch_from_rotation(rotation_matrix)
    roll = roll_from_rotation(rotation_matrix)
    return yaw, pitch, roll


def yaw_from_rotation(rotation_matrix: np.ndarray) -> float:
    Rt_ez = np.dot(rotation_matrix.T, [0, 0, 1])
    return np.arctan2(Rt_ez[0], Rt_ez[1])


def pitch_from_rotation(rotation_matrix: np.ndarray) -> float:
    Rt_ez = np.dot(rotation_matrix.T, [0, 0, 1])
...
def rotation_from_ptr_v2(yaw: float, pitch: float, roll: float) -> np.ndarray:
    """Camera rotation matrix from yaw, pitch and roll.

    This is the implementation used in the Single Image Calibration code.
    """
    pitch += np.pi / 2
    return transformations.euler_matrix(yaw, pitch, roll, "szxz")[:3, :3]
...
def ptr_from_rotation_v2(rotation_matrix: np.ndarray) -> Tuple[float, float, float]:
    ...
    yaw, pitch, roll = transformations.euler_from_matrix(T, "szxz")
    return yaw, pitch - np.pi / 2, roll


def rotation_from_opk(omega: float, phi: float, kappa: float) -> np.ndarray:
    """World-to-camera rotation matrix from yaw, pitch and roll."""
